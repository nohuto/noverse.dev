...
You can edit the settings the script applies via `$settings`, to do so you need to know if a key is in the root block or if it's in a specific block. The script currently applies several settings which are listed in [#app-settings](/docs/app-guides/steam/#app-settings).
...
Means to automate the configuration we only use `localconfig.vdf`/`sharedconfig.vdf`. It's not possible to configure all settings from [#app-settings](/docs/app-guides/steam/#app-settings) via the local files/registry but some of them.
