...
If extracting the whole System32 folder recursively using [strings2](/docs/regkit/sections/overview/) you'll see that the string `defaultuser0` only exists in `DeviceEnroller.exe` (at least for me). The notes below are based on several decompiled functions from `DeviceEnroller.exe` (`MakeActiveUserLocalAdmin`).
