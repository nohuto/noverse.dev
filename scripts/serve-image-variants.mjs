  ...
  return {
    ...
    configureServer(server) {
      ...
      server.watcher.on('change', (file) => {
        if (file.startsWith(imageSources) || file.startsWith(publicImages))
          cache.clear();
      });
      server.middlewares.use(async (request, response, next) => {
        ...
        const match = pathname.match(
          /^\/main\/images\/([\w./-]+)-(480|640|960)\.webp$/,
        );
        ...
        const original = existsSync(source)
          ? source
          : path.resolve(publicImages, `${match[1]}.png`);
        try {
          let data = cache.get(pathname);
          if (!data) {
            data = await sharp(original)
              .resize({ width: Number(match[2]), withoutEnlargement: true })
